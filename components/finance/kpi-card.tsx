...
export function KpiCard({ label, value, hint, emphasis = "default" }: KpiCardProps) {
  return (
    <div className="rounded-lg border border-border-subtle bg-surface p-6">
      <p className="text-caption uppercase tracking-wider text-graphite">{label}</p>
      <p
        className={cn(
          "mt-2 flex items-center gap-1.5 font-display text-h2 tabular-nums",
          emphasis === "positive" ? "text-forest" : "text-navy",
      ...
      {hint && <p className="mt-1 text-caption text-graphite">{hint}</p>}
    </div>
  );
}
